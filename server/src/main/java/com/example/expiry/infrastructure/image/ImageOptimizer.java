...
public class ImageOptimizer {
    ...
    private static final int MAX_DIMENSION = 1280;   // scale down nếu lớn hơn
    private static final float JPEG_QUALITY = 0.80f; // 0..1

    /**
     * @param originalBytes bytes gốc từ MultipartFile
     * @param originalContentType file.getContentType() (image/jpeg, image/png, image/webp)
     * @return OptimizedImage (bytes + mimeType)
     */
    public OptimizedImage optimize(byte[] originalBytes, String originalContentType) {
        ...
        // Try decode. If fail (ex: webp), just return original.
        BufferedImage src;
        ...
        if (src == null) {
            // ImageIO cannot decode -> keep original
            return new OptimizedImage(originalBytes, safeMime(originalContentType));
        }

        BufferedImage scaled = scaleDownIfNeeded(src, MAX_DIMENSION);

        // Always encode to JPEG for smaller + faster (ok for food package photos)
        byte[] jpegBytes = encodeJpeg(scaled, JPEG_QUALITY);
    ...
    private BufferedImage scaleDownIfNeeded(BufferedImage src, int maxDim) {
        ...
        if (max <= maxDim) {
            return toRgb(src); // still convert to RGB for stable JPEG output
        }
    ...
    /**
     * Convert any image (including with alpha) into RGB with white background.
     */
    ...
    private byte[] encodeJpeg(BufferedImage img, float quality) {
        ...
        if (!writers.hasNext()) {
            // should not happen on normal JDK
            try {
